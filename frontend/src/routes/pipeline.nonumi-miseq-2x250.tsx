...
import {
  buildNonUmi2x250Payload,
  NONUMI_2X250_PROGRESS_STEPS,
} from "@/pipelines/nonumimiseq2x250";

export const Route = createFileRoute("/pipeline/nonumi-miseq-2x250")({
  head: () => ({
    meta: [
      { title: "Illumina MiSeq 2×250 BCR mRNA — Pipeline" },
      {
        name: "description",
        content:
          "Preprocessing workflow for paired-end bulk BCR mRNA repertoire data without UMI barcoding.",
      },
    ],
  }),
...
function PipelinePage() {
  ...
  const progress = useJobProgress(jobId, NONUMI_2X250_PROGRESS_STEPS.length);
  ...
  const launch = useJobLaunch({
    ...
    start: async () => {
      ...
      return startJobMutation.mutateAsync({
        ...
        route: "/pipeline/nonumi-miseq-2x250",
        label: "Non-UMI MiSeq 2×250",
      });
    },
    onStarted: (result) => {
          ...
          >
            ...
            >
              <div className="w-full max-w-2xl">
                <LibrarySchematic variant="nonumi" />
              </div>
            </div>
          </div>
        </section>
          ...
          >
            {jobId && (
              <div className="flex min-w-0 flex-col gap-4">
                <PipelineProgress
                  progress={progress}
                  steps={NONUMI_2X250_PROGRESS_STEPS}
                  jobId={jobId}
                  onReset={handleReset}
                />
                ...
                {FLOW.map((node, idx) => {
                  ...
                  return (
                    <div key={node.id} className="flex w-full flex-col items-center">
                      <FlowNodeBox node={node} uploaded={uploaded} hovered={hovered} />
...
function ConvergeArrows({ active }: { active: boolean }) {
  ...
  return (
    <svg viewBox="0 0 200 40" className="my-1 h-10 w-full max-w-md" aria-hidden>
      <defs>
        <marker
          id="arrowhead-converge-nonumi"
          viewBox="0 0 10 10"
          refX="8"
          refY="5"
          markerWidth="6"
      ...
      <path
        ...
        fill="none"
        markerEnd="url(#arrowhead-converge-nonumi)"
      />
      <path
...
function FlowArrow({ active }: { active: boolean }) {
  return (
    <div className="flex flex-col items-center">
      <div className={cn("h-4 w-px transition-colors", active ? "bg-primary/60" : "bg-border")} />
      <div
        className={cn(
